...
(function() {
  ...
  function animateNumber(el) {
    ...
    const match = textOnly.match(/^(\d[\d,]*)/);
    if (!match) return;
    ...
    const numInHtml = html.indexOf(match[1]);
    if (numInHtml === -1) return;

    const beforeNum = html.substring(0, numInHtml);
    const afterNum = html.substring(numInHtml + match[1].length);
  ...
  function initCtaShine() {
    const ctaBtns = document.querySelectorAll('.hero-cta-group .btn, .btn-primary.btn-lg');
  ...
  // ─── 10. 초기화 (각 모듈 독립 실행 — 하나가 실패해도 나머지 정상 동작) ───
  function safeRun(fn, name) {
    try { fn(); } catch (e) { console.warn('[micro.js] ' + name + ' error:', e.message); }
  }

  function init() {
    setTimeout(function() {
      // v1.0
      safeRun(initScrollReveal, 'scrollReveal');
      safeRun(initCountUp, 'countUp');
      safeRun(enhanceHeader, 'header');
      safeRun(enhanceScrollTop, 'scrollTop');
      // v2.0
      safeRun(initReviewStagger, 'reviewStagger');
      safeRun(initMouseGlow, 'mouseGlow');
      safeRun(initHeroEntrance, 'heroEntrance');
      safeRun(initCtaShine, 'ctaShine');
      safeRun(initScrollProgress, 'scrollProgress');
    }, 100);
  }
