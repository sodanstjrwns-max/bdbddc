...
(function() {
  ...
  function createLightbox() {
    ...
    if (!document.getElementById('lbStylesV17')) {
      ...
      s.textContent =
        ...
        /* ── #1 비슷한 케이스 추천 ── */
        '.lb-similar-section{margin-top:14px;padding-top:14px;border-top:1px solid rgba(255,255,255,0.06);}' +
        '.lb-similar-header{font-size:0.8rem;font-weight:700;color:rgba(255,255,255,0.5);margin-bottom:10px;display:flex;align-items:center;gap:6px;}' +
        '.lb-similar-header i{color:#C8A97E;font-size:0.7rem;}' +
        '.lb-similar-grid{display:flex;gap:10px;overflow-x:auto;scrollbar-width:none;-ms-overflow-style:none;padding-bottom:4px;}' +
        '.lb-similar-grid::-webkit-scrollbar{display:none;}' +
        '.lb-similar-card{flex:0 0 auto;width:160px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.06);border-radius:12px;overflow:hidden;transition:all .25s;}' +
        '.lb-similar-card:hover{background:rgba(255,255,255,0.08);border-color:rgba(201,169,126,0.25);transform:translateY(-2px);}' +
        '.lb-similar-img{width:100%;aspect-ratio:4/3;overflow:hidden;background:#1a1714;}' +
        '.lb-similar-img img{width:100%;height:100%;object-fit:cover;}' +
        '.lb-similar-ph{width:100%;height:100%;display:flex;align-items:center;justify-content:center;color:rgba(255,255,255,0.12);font-size:1.4rem;}' +
        '.lb-similar-info{padding:8px 10px;}' +
        '.lb-similar-title{font-size:0.75rem;font-weight:600;color:rgba(255,255,255,0.8);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-bottom:4px;}' +
        '.lb-similar-meta{display:flex;flex-wrap:wrap;gap:6px;font-size:0.65rem;color:rgba(255,255,255,0.4);}' +
        '.lb-similar-meta span{display:flex;align-items:center;gap:3px;}' +
        '.lb-similar-meta i{color:#C8A97E;font-size:0.55rem;}' +
        '@media(max-width:600px){.lb-similar-card{width:140px;}.lb-similar-title{font-size:0.7rem;}.lb-similar-meta{font-size:0.6rem;}}';
  ...
  function openLightbox(caseItem, startType) {
    ...
    // #8 CTA — 케이스 정보를 쿼리 파라미터로 예약 페이지에 전달
    var reserveParams = '?ref_case=' + encodeURIComponent(caseItem.id) +
      '&ref_title=' + encodeURIComponent(caseItem.title || catLabel + ' 케이스') +
      '&ref_doctor=' + encodeURIComponent(caseItem.doctorName || '') +
      '&ref_category=' + encodeURIComponent(catLabel);
    var ctaHtml = '<a href="/reservation' + reserveParams + '" class="lb-info-cta" onclick="event.stopPropagation()"><i class="fas fa-calendar-check"></i> 이 케이스처럼 상담받기</a>';

    // #1 비슷한 케이스 추천 (같은 카테고리, 자기 자신 제외, 최대 3개)
    var similarHtml = '';
    if (cases && cases.length > 1) {
      var similar = cases.filter(function(sc) {
        return sc.category === caseItem.category && sc.id !== caseItem.id;
      });
      // 비슷한 조건 우선 (같은 성별·나이대)
      similar.sort(function(a, b) {
        var scoreA = (a.patientGender === caseItem.patientGender ? 2 : 0) + (a.patientAge === caseItem.patientAge ? 1 : 0);
        var scoreB = (b.patientGender === caseItem.patientGender ? 2 : 0) + (b.patientAge === caseItem.patientAge ? 1 : 0);
        return scoreB - scoreA;
      });
      var topSimilar = similar.slice(0, 3);
      if (topSimilar.length > 0) {
        var cardsHtml = '';
        topSimilar.forEach(function(sc) {
          var scImg = sc.beforeImage || sc.thumbnailImage || '';
          var scCatIcon = CAT_ICONS[sc.category] || 'fa-tooth';
          var scPatient = [sc.patientAge || '', sc.patientGender === 'male' ? '남성' : sc.patientGender === 'female' ? '여성' : ''].filter(Boolean).join(' ');
          cardsHtml +=
            '<div class="lb-similar-card" data-case-id="' + sc.id + '" style="cursor:pointer">' +
              '<div class="lb-similar-img">' +
                (scImg ? '<img src="' + scImg + '" alt="' + (sc.title || '') + '" loading="lazy">' : '<div class="lb-similar-ph"><i class="fas ' + scCatIcon + '"></i></div>') +
              '</div>' +
              '<div class="lb-similar-info">' +
                '<div class="lb-similar-title">' + (sc.title || '케이스') + '</div>' +
                '<div class="lb-similar-meta">' +
                  (sc.doctorName ? '<span><i class="fas fa-user-md"></i> ' + sc.doctorName + '</span>' : '') +
                  (scPatient ? '<span><i class="fas fa-user"></i> ' + scPatient + '</span>' : '') +
                '</div>' +
              '</div>' +
            '</div>';
        });
        similarHtml =
          '<div class="lb-similar-section">' +
            '<div class="lb-similar-header"><i class="fas fa-th-large"></i> 비슷한 케이스</div>' +
            '<div class="lb-similar-grid">' + cardsHtml + '</div>' +
          '</div>';
      }
    }
    ...
    // 비슷한 케이스 카드 클릭 이벤트
    setTimeout(function() {
      var simCards = infoEl.querySelectorAll('.lb-similar-card');
      simCards.forEach(function(card) {
        card.addEventListener('click', function(e) {
          e.stopPropagation();
          var targetId = card.getAttribute('data-case-id');
          var targetCase = cases.find(function(c) { return c.id === targetId; });
          if (targetCase) {
            openLightbox(targetCase, 'before');
          }
        });
      });
    }, 50);
