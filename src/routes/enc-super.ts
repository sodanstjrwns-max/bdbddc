...
<h3>나이별 영구치 맹출 시기표 — 우리 아이는 지금 어디쯤?</h3>
<p>아래 표는 일반적인 평균 시기입니다. 아이마다 6개월~1년의 개인차는 정상 범위이니, 순서의 큰 흐름이 맞는지를 기준으로 보시면 됩니다.</p>
<table style="width:100%;border-collapse:collapse;margin:14px 0;">
<thead><tr>
<th style="${TH}">치아</th><th style="${TH}">아래턱</th><th style="${TH}">위턱</th><th style="${TH}">체크 포인트</th>
</tr></thead>
<tbody>
<tr><td style="${TD}"><strong>제1대구치(6세 구치)</strong></td><td style="${TDC}">6~7세</td><td style="${TDC}">6~7세</td><td style="${TD}">빠지는 유치 없이 맨 뒤에 남 → ${A('실란트')} 필수</td></tr>
<tr><td style="${TD}"><strong>중절치(앞니)</strong></td><td style="${TDC}">6~7세</td><td style="${TDC}">7~8세</td><td style="${TD}">가장 먼저 흔들리는 치아. 크고 누렇게 보여도 정상</td></tr>
<tr><td style="${TD}"><strong>측절치(옆 앞니)</strong></td><td style="${TDC}">7~8세</td><td style="${TDC}">8~9세</td><td style="${TD}">'못난이 시기' — 벌어져 나도 대부분 자연 정돈</td></tr>
<tr><td style="${TD}"><strong>제1소구치</strong></td><td style="${TDC}">10~12세</td><td style="${TDC}">10~11세</td><td style="${TD}">유구치와 교체. ${A('소구치')} 참고</td></tr>
<tr><td style="${TD}"><strong>${A('견치', '견치(송곳니)')}</strong></td><td style="${TDC}">9~10세</td><td style="${TDC}">11~12세</td><td style="${TD}">공간 부족 시 <strong>덧니</strong>로 가장 잘 튀어나오는 치아</td></tr>
<tr><td style="${TD}"><strong>제2소구치</strong></td><td style="${TDC}">11~12세</td><td style="${TDC}">10~12세</td><td style="${TD}">선천적 결손이 비교적 흔한 부위</td></tr>
<tr><td style="${TD}"><strong>제2대구치(12세 구치)</strong></td><td style="${TDC}">11~13세</td><td style="${TDC}">12~13세</td><td style="${TD}">이갈이 마무리. 깊숙해 양치가 어려움</td></tr>
<tr><td style="${TD}"><strong>${A('사랑니', '제3대구치(사랑니)')}</strong></td><td style="${TDC}" colspan="2">17~25세</td><td style="${TD}">없거나 매복되는 경우도 많음</td></tr>
</tbody></table>

<div style="${BOX}">
<strong>부모님이 자주 놀라시는 3가지 — 대부분 정상입니다</strong>
<ul style="margin:8px 0 0;padding-left:18px;">
<li><strong>"새로 난 앞니가 누레요"</strong> → 영구치는 유치보다 상아질이 두꺼워 원래 약간 누런빛입니다. 하얀 유치와 나란히 있으니 더 도드라져 보일 뿐입니다.</li>
<li><strong>"앞니가 너무 커요"</strong> → 영구치는 성인 크기로 나옵니다. 얼굴이 자라면서 비율이 맞아갑니다.</li>
<li><strong>"앞니가 벌어져서 났어요"</strong> → 옆 앞니와 송곳니가 나면서 밀려 대부분 좁혀집니다(일명 '미운 오리 시기'). 다만 만 9세 이후에도 유지되면 검진을 권합니다.</li>
</ul>
</div>
...
<div style="${WARN}">
<strong>이럴 땐 미루지 말고 치과에 오세요</strong><br>
① 또래보다 <strong>1년 이상</strong> 교체가 늦을 때(영구치 결손·매복 가능성) ② 유치가 <strong>충치로 일찍 빠졌을 때</strong>(뒤 치아가 앞으로 쓰러져 영구치 자리를 잠식 → 공간유지장치 필요) ③ 흔들리지 않는 유치가 <strong>통증·잇몸 붓기</strong>를 동반할 때 ④ 좌우 대칭이 심하게 어긋날 때. 이갈이 시기의 공간 관리는 나중의 교정 난이도와 비용을 크게 좌우합니다.
</div>

<h3>이갈이 시기 양치 — 난이도가 갑자기 올라갑니다</h3>
<p>혼합 치열기(유치와 영구치가 섞여 있는 시기)는 <strong>평생 중 충치 위험이 가장 높은 구간</strong>입니다. 치아 높이가 들쭉날쭉해 칫솔모가 닿지 않는 사각지대가 생기기 때문입니다. 특히 새로 난 6세 구치는 키가 낮아 칫솔이 그냥 지나가기 쉬우니, <strong>칫솔을 옆에서 가로로 넣어 어금니 씹는 면을 따로 닦는</strong> 습관을 들여야 합니다. 만 8~10세까지는 아이가 혼자 닦은 뒤 <strong>부모가 한 번 더 마무리</strong>해 주는 것이 가장 확실한 예방법이며, 3~6개월 간격의 정기검진과 ${A('불소 도포')}를 병행하면 효과가 큽니다.</p>`,
  faqs: [
    ...
    { q: '새로 난 영구치가 유치보다 누런데 괜찮나요?', a: '정상입니다. 영구치는 유치보다 상아질이 두껍고 법랑질이 반투명해 원래 약간 누런빛을 띱니다. 하얀 유치와 나란히 있어 더 도드라져 보일 뿐이며, 착색이나 충치가 아닙니다. 다만 특정 부위만 하얗게 뜨거나 갈색 점이 있다면 검진을 받아보세요.' },
    { q: '유치가 충치로 일찍 빠졌는데 어차피 빠질 치아 아닌가요?', a: '아닙니다. 유치는 영구치가 나올 자리를 지켜주는 역할을 합니다. 일찍 빠지면 뒤쪽 치아가 앞으로 쓰러져 공간이 좁아지고, 나중에 영구치가 덧니로 나거나 매복될 수 있습니다. 이런 경우 공간유지장치가 필요하니 반드시 치과 상담을 받으세요.' },
    { q: '이갈이 시기에 양치는 어떻게 도와줘야 하나요?', a: '혼합 치열기는 치아 높이가 들쭉날쭉해 충치 위험이 가장 높은 시기입니다. 특히 새로 난 6세 구치는 키가 낮아 칫솔이 지나치기 쉬우니 칫솔을 가로로 넣어 씹는 면을 따로 닦아주세요. 만 8~10세까지는 아이가 닦은 뒤 부모가 마무리해 주고, 3~6개월 간격 검진과 불소도포를 병행하면 좋습니다.' },
  ],
}
...
<h3>인비절라인 패키지와 비용 — 왜 견적이 사람마다 다를까?</h3>
<p>인비절라인은 하나의 가격이 아니라 <strong>치아를 얼마나 많이, 얼마나 크게 움직이느냐(=얼라이너 개수)</strong>에 따라 패키지가 나뉩니다. 같은 '투명교정'이라도 견적 차이가 큰 이유가 여기 있습니다.</p>
<table style="width:100%;border-collapse:collapse;margin:14px 0;">
<thead><tr>
<th style="${TH}">패키지</th><th style="${TH}">적합한 케이스</th><th style="${TH}">비용</th>
</tr></thead>
<tbody>
<tr><td style="${TD}"><strong>익스프레스</strong></td><td style="${TD}">아주 경미한 재배열, 교정 후 재발 정리</td><td style="${TDC}">300만원</td></tr>
<tr><td style="${TD}"><strong>퍼스트</strong></td><td style="${TD}">성장기 어린이(혼합 치열) 1차 교정</td><td style="${TDC}">400만원</td></tr>
<tr><td style="${TD}"><strong>라이트</strong></td><td style="${TD}">가벼운 덧니·앞니 틈, 부분적 개선</td><td style="${TDC}">450만원</td></tr>
<tr><td style="${TD}"><strong>모더레이트</strong></td><td style="${TD}">중등도 총생·공간, 중간 난이도</td><td style="${TDC}">550만원</td></tr>
<tr><td style="${TD}"><strong>컴프리헨시브</strong></td><td style="${TD}">전악 교정, 발치 케이스 등 고난도</td><td style="${TDC}">700만원</td></tr>
<tr><td style="${TD}"><strong>유지장치(리테이너)</strong></td><td style="${TD}">교정 종료 후 필수 — 양악 / 편악</td><td style="${TDC}">25만원 / 15만원</td></tr>
</tbody></table>
<p style="font-size:0.82rem;color:#8a7a68;">※ 서울비디치과 공시 수가 기준. 최종 패키지는 X-ray·구강스캔 진단 후 결정되며, 교정은 건강보험이 적용되지 않는 비급여 항목입니다.</p>

<h3>인비절라인 치료는 이렇게 진행됩니다</h3>
<ol>
<li><strong>진단·구강스캔</strong> — 파노라마 X-ray와 3D 스캐너로 현재 치열을 기록합니다. 본뜨는 인상재 없이 스캔으로 대체돼 훨씬 편합니다.</li>
<li><strong>시뮬레이션(클린체크) 확인</strong> — 치료 후 예상 치열을 3D 영상으로 미리 봅니다. 인비절라인의 가장 큰 장점으로, <strong>시작 전에 결과를 눈으로 확인</strong>할 수 있습니다.</li>
<li><strong>얼라이너 제작·부착물 세팅</strong> — 전용 얼라이너가 도착하면 치아 이동을 돕는 어태치먼트(작은 돌기)를 붙입니다.</li>
<li><strong>1~2주 간격 교체</strong> — 정해진 순서대로 얼라이너를 교체하며 치아를 이동시킵니다. 내원은 보통 6~10주 간격으로, 기존 교정보다 방문 횟수가 적습니다.</li>
<li><strong>마무리·유지장치</strong> — 목표 치열에 도달하면 리파인먼트(미세 조정) 후 ${A('유지장치')}를 착용합니다.</li>
</ol>

<div style="${BOX}">
<strong>인비절라인에 대한 흔한 오해</strong>
<ul style="margin:8px 0 0;padding-left:18px;">
<li><strong>"투명교정은 안 아프다"</strong> → 통증은 적은 편이지만 <strong>새 얼라이너로 바꾼 첫 2~3일은 뻐근함</strong>이 있습니다. 이 압박감이 치아가 움직이고 있다는 신호입니다.</li>
<li><strong>"아무것도 안 붙인다"</strong> → 대부분 어태치먼트가 치아 표면에 붙습니다. 치아색이라 눈에 잘 띄지는 않지만 '완전 무부착'은 아닙니다.</li>
<li><strong>"기간이 훨씬 짧다"</strong> → 케이스가 같다면 기간은 고정식과 크게 다르지 않습니다. 짧게 끝나는 것은 대개 난이도가 낮은 케이스입니다.</li>
</ul>
</div>
...
<h3>얼라이너 관리 — 이것만 지키면 실패하지 않습니다</h3>
<ul>
<li><strong>뜨거운 물 금지</strong>: 얼라이너는 열에 변형됩니다. 소독한다고 끓는 물에 넣으면 그대로 못 쓰게 되니 <strong>미지근한 물과 부드러운 칫솔</strong>로 세척하세요.</li>
<li><strong>물 외의 음료는 빼고</strong>: 커피·주스를 낀 채로 마시면 얼라이너 안에 당분과 색소가 갇혀 <strong>충치와 착색</strong>을 동시에 부릅니다. 물만 예외입니다.</li>
<li><strong>먹은 뒤 양치 후 착용</strong>: 음식물 위에 얼라이너를 씌우면 밀폐된 상태로 세균이 증식합니다. 외출 시 휴대용 칫솔이 필수입니다.</li>
<li><strong>이전 단계 얼라이너 보관</strong>: 분실했을 때 한 단계 되돌아가 착용하면 시간 손실을 줄일 수 있습니다. 버리지 말고 보관하세요.</li>
<li><strong>취침 중에도 착용</strong>: 하루 20~22시간을 채우는 가장 쉬운 방법이 수면 시간 활용입니다.</li>
</ul>

<h3>교정이 끝난 뒤가 진짜 시작 — 유지장치</h3>
<p>수백만원과 1~2년을 들인 교정 결과를 지키는 것은 <strong>${A('유지장치')}(리테이너)</strong>입니다. 치아는 이동한 위치에서 원래 자리로 돌아가려는 성질(재발)이 강하고, 특히 <strong>교정 직후 6개월~1년이 가장 취약</strong>합니다. 초기에는 식사·양치 때를 제외하고 종일 착용하다가 이후 야간 착용으로 줄여가며, 사실상 평생 주 2~3회 야간 착용을 권합니다. 서울비디치과 유지장치는 양악 25만원, 편악 15만원입니다. 재발해서 재교정을 받는 비용에 비하면 훨씬 경제적인 보험입니다.</p>`,
  faqs: [
    ...
    { q: '서울비디치과 인비절라인 비용은 패키지별로 얼마인가요?', a: '익스프레스 300만원, 퍼스트(어린이) 400만원, 라이트 450만원, 모더레이트 550만원, 컴프리헨시브 700만원입니다. 교정 종료 후 필요한 유지장치는 양악 25만원, 편악 15만원입니다. 패키지는 치아를 얼마나 많이 움직이는지(얼라이너 개수)에 따라 진단 후 결정되며, 교정은 비급여 항목입니다.' },
    { q: '인비절라인은 결과를 미리 볼 수 있나요?', a: '네. 3D 구강스캔 후 클린체크라는 시뮬레이션으로 치료 후 예상 치열을 영상으로 미리 확인할 수 있습니다. 시작 전에 결과 방향을 눈으로 보고 결정할 수 있다는 점이 인비절라인의 큰 장점입니다. 다만 시뮬레이션은 목표 예측치이며, 실제 결과는 착용 시간 준수 여부에 따라 달라집니다.' },
    { q: '인비절라인은 병원에 얼마나 자주 가야 하나요?', a: '얼라이너는 집에서 1~2주 간격으로 직접 교체하고, 내원은 보통 6~10주 간격입니다. 매달 와이어를 조정해야 하는 고정식 교정보다 방문 횟수가 적어 직장인·학생에게 유리합니다. 다만 진행 상태 점검은 반드시 예정대로 받으셔야 합니다.' },
...
<h3>서울비디치과 미백 비용과 소요 시간 — 한눈에 비교</h3>
<p>미백은 '몇 번, 얼마나 오래, 얼마에'가 가장 궁금한 항목입니다. 서울비디치과에서 진행하는 전문가 미백은 아래 두 가지로 정리됩니다.</p>
<table style="width:100%;border-collapse:collapse;margin:14px 0;">
<thead><tr>
<th style="${TH}">구분</th><th style="${TH}">소프트 블리칭</th><th style="${TH}">하드 블리칭</th>
</tr></thead>
<tbody>
<tr><td style="${TD}"><strong>내원 횟수</strong></td><td style="${TDC}">1회</td><td style="${TDC}">2회</td></tr>
<tr><td style="${TD}"><strong>시술 시간</strong></td><td style="${TDC}">약 30분</td><td style="${TDC}">약 60분</td></tr>
<tr><td style="${TD}"><strong>비용</strong></td><td style="${TDC}"><strong>4.9만원</strong></td><td style="${TDC}"><strong>8만원</strong></td></tr>
<tr><td style="${TD}"><strong>이런 분께</strong></td><td style="${TD}">가벼운 착색, 행사·면접 전 빠른 정돈</td><td style="${TD}">누적 착색이 뚜렷하고 확실한 톤 변화를 원할 때</td></tr>
<tr><td style="${TD}"><strong>시림 정도</strong></td><td style="${TDC}">적은 편</td><td style="${TDC}">상대적으로 큼(대부분 며칠 내 소실)</td></tr>
</tbody></table>
<p style="font-size:0.82rem;color:#8a7a68;">※ 서울비디치과 공시 수가 기준(비보험). 치아 상태·착색 원인에 따라 권장 방식이 달라질 수 있습니다.</p>

<div style="${BOX}">
<strong>미백에 대한 가장 흔한 오해 3가지</strong>
<ul style="margin:8px 0 0;padding-left:18px;">
<li><strong>"미백하면 치아가 깎인다"</strong> → 아닙니다. 미백은 치아를 삭제하지 않고 약제로 색소만 분해합니다. 치아를 깎는 것은 ${A('라미네이트')}·크라운 쪽입니다.</li>
<li><strong>"한 번 하면 평생 하얗다"</strong> → 아닙니다. 평균 1~2년이며 커피·흡연 습관에 따라 6개월 만에 되돌아가기도 합니다. 유지 관리가 절반입니다.</li>
<li><strong>"연예인처럼 새하얗게 된다"</strong> → 자연치의 미백 한계는 사람마다 다릅니다. 화면 속의 균일한 백색은 대부분 보철물(라미네이트)입니다.</li>
</ul>
</div>

<h3>미백 전에 반드시 체크할 3가지</h3>
<p>미백은 '색'만 다루는 시술이라, 치아 자체에 문제가 있으면 통증과 실패로 이어집니다. 시술 전 아래 3가지는 반드시 확인해야 합니다.</p>
<ul>
<li><strong>충치·균열 확인</strong>: ${A('충치')}나 ${A('치아 균열')}이 있으면 약제가 내부로 스며들어 심한 통증을 유발합니다. 치료를 먼저 끝내야 합니다.</li>
<li><strong>잇몸 상태 확인</strong>: ${A('치은염')}·${A('치주염')}이 있으면 잇몸이 약제에 자극받습니다. ${A('스케일링')}으로 정리한 뒤 진행하는 것이 안전합니다.</li>
<li><strong>보철물 위치 확인</strong>: 앞니에 ${A('레진')}·크라운이 있다면 미백 후 색 차이가 드러납니다. 미백 → 색 안정(약 2주) → 보철물 교체 순서가 정석입니다.</li>
</ul>
...
<h3>미백 후 시림과 관리 — 첫 48시간이 승부처</h3>
<p>미백 후 며칠간 <strong>이가 시린 증상</strong>은 흔하며 대부분 자연히 가라앉습니다. 문제는 색입니다. 미백 직후 치아 표면의 미세한 통로가 일시적으로 열려 있어, <strong>이 시기에 먹는 색소가 그대로 각인</strong>됩니다. 그래서 첫 48시간 관리가 1~2년의 유지력을 좌우합니다.</p>
<table style="width:100%;border-collapse:collapse;margin:14px 0;">
<thead><tr>
<th style="${TH}">시기</th><th style="${TH}">피해야 할 것</th><th style="${TH}">권장</th>
</tr></thead>
<tbody>
<tr><td style="${TD}"><strong>직후 ~48시간</strong></td><td style="${TD}">커피·홍차·와인·콜라·카레·김치·베리류, 흡연</td><td style="${TD}">물·우유·흰 쌀밥·닭가슴살 등 '화이트 다이어트'</td></tr>
<tr><td style="${TD}"><strong>1주 이내</strong></td><td style="${TD}">진한 색소 음료 다량 섭취, 지나치게 찬 음식</td><td style="${TD}">시린 증상 시 시린이용 치약 사용</td></tr>
<tr><td style="${TD}"><strong>이후 유지기</strong></td><td style="${TD}">흡연, 하루 여러 잔의 커피를 물 없이</td><td style="${TD}">색소 음료 후 물 헹굼, 6개월~1년마다 ${A('스케일링')}</td></tr>
</tbody></table>

<div style="${WARN}">
<strong>이런 경우는 미백을 미루세요</strong><br>
임신·수유 중이거나 만 15세 미만(치수가 커서 시림이 심함), 치료되지 않은 충치·잇몸 질환이 있는 경우, 잇몸이 심하게 내려가 치아 뿌리(백악질)가 노출된 경우에는 미백을 권장하지 않습니다. 상태를 먼저 정리한 뒤 진행하는 것이 결과와 안전 모두에 유리합니다.
</div>

<p>정리하면, 미백은 <strong>깎지 않고 인상을 바꿀 수 있는 가장 가성비 좋은 심미 치료</strong>입니다. 다만 '한 번에 영구히'가 아니라 <strong>초기 시술 + 생활 관리 + 주기적 보충</strong>이라는 관리형 시술로 접근할 때 만족도가 가장 높습니다. 착색 원인이 무엇인지, 미백으로 해결 가능한 색인지부터 확인하는 것이 첫 단계입니다.</p>`,
  faqs: [
    ...
    { q: '서울비디치과 치아 미백 비용은 얼마인가요?', a: '소프트 블리칭은 1회 약 30분에 4.9만원, 하드 블리칭은 2회 약 60분에 8만원입니다(비보험). 가벼운 착색이나 행사 전 빠른 정돈에는 소프트, 누적 착색이 뚜렷해 확실한 톤 변화를 원하면 하드 블리칭을 권장합니다. 치아 상태에 따라 권장 방식이 달라질 수 있어 상담 후 결정합니다.' },
    { q: '미백 후 커피는 언제부터 마실 수 있나요?', a: '최소 48시간은 피하는 것이 좋습니다. 미백 직후에는 치아 표면의 미세 통로가 일시적으로 열려 있어 이때 섭취한 색소가 그대로 착색됩니다. 첫 이틀은 물·우유·흰 쌀밥 등 색이 옅은 음식 위주로 드시고, 이후에도 커피를 마신 뒤 물로 헹구면 유지력이 크게 좋아집니다.' },
  ],
}
...
<h3>보험 스케일링 vs 비보험 스케일링 — 뭐가 다른가요?</h3>
<p>가장 많이 헷갈리는 부분입니다. <strong>시술 자체가 다른 게 아니라 '적용 기준'이 다릅니다.</strong> 아래 표로 정리했습니다.</p>
<table style="width:100%;border-collapse:collapse;margin:14px 0;">
<thead><tr>
<th style="${TH}">구분</th><th style="${TH}">보험 스케일링</th><th style="${TH}">비보험 스케일링</th>
</tr></thead>
<tbody>
<tr><td style="${TD}"><strong>대상</strong></td><td style="${TD}">만 19세 이상</td><td style="${TD}">나이 제한 없음</td></tr>
<tr><td style="${TD}"><strong>횟수</strong></td><td style="${TD}">연 1회 (1/1~12/31)</td><td style="${TD}">제한 없음</td></tr>
<tr><td style="${TD}"><strong>본인부담</strong></td><td style="${TDC}">약 1만~2만원</td><td style="${TDC}">6만원</td></tr>
<tr><td style="${TD}"><strong>적용 시점</strong></td><td style="${TD}">그 해 첫 예방 스케일링</td><td style="${TD}">연 1회 소진 후 추가분</td></tr>
</tbody>
</table>
<p style="font-size:0.82rem;color:#8a7a66;margin-top:-6px;">※ 서울비디치과 비보험 스케일링 6만원 기준. 보험 본인부담금은 치과·잇몸 상태에 따라 달라질 수 있습니다.</p>

<div style="${BOX}">
<strong>💡 가장 흔한 오해 3가지</strong>
<p style="margin:8px 0 0;font-size:0.88rem;line-height:1.8;">
① <strong>"작년 12월에 받았으니 올해 12월까지 기다려야 한다"</strong> → ❌ 아닙니다. 기준은 <strong>연도</strong>라 1월 1일이면 즉시 초기화됩니다. 12월에 받고 다음 달 1월에 또 받아도 보험 적용됩니다.<br>
② <strong>"스케일링 받으면 이가 시리고 벌어진다"</strong> → 치석이 빠진 자리가 드러나 일시적으로 시릴 수 있지만, 보통 <strong>1~2주면 가라앉습니다.</strong> 이가 벌어진 게 아니라 잇몸병으로 이미 녹아 있던 공간이 보이는 것입니다.<br>
③ <strong>"잇몸이 안 아프면 안 받아도 된다"</strong> → 잇몸병은 <strong>통증 없이 진행</strong>됩니다. 아플 땐 이미 뼈가 녹은 뒤입니다.
</p>
</div>

<h3>스케일링 받는 최적 타이밍은 언제일까?</h3>
<p>실무적으로 <strong>연말(11~12월)에 몰립니다.</strong> "혜택 날아가기 전에" 오시기 때문이죠. 하지만 그 시기는 예약이 가장 어렵습니다. <strong>연초(1~3월)에 받으시는 걸 권합니다.</strong> 혜택은 1월 1일에 이미 초기화돼 있고, 예약도 여유롭고, 잇몸 문제가 발견되면 그 해 안에 치료할 시간도 충분합니다.</p>
<p>또 한 가지 — <strong>스케일링은 국가 건강검진과 별개입니다.</strong> 검진에서 "구강검진 이상 없음"이 나와도 스케일링 혜택은 그대로 남아 있습니다. 두 개를 같은 것으로 알고 넘기시는 분이 의외로 많습니다.</p>

<h3>치주 치료 목적이면 연 1회와 별개입니다</h3>
<p>이 부분이 실제로 가장 큰 혜택인데 잘 안 알려져 있습니다. 잇몸에서 피가 나거나 ${A('치주염')} 진단을 받으면, 그건 <strong>'예방'이 아니라 '치료'</strong>입니다. 치료 목적 치석 제거는 예방 연 1회 한도와 <strong>무관하게</strong> 보험이 적용됩니다. 잇몸 상태에 따라 ${A('치근활택술')}·${A('치주소파술')} 단계로 이어지며, 이 역시 보험 항목입니다.</p>
<p>즉 <strong>"올해 스케일링 이미 받았으니 잇몸 아파도 참아야지"는 완전한 오해</strong>입니다. 증상이 있으면 그건 다른 트랙입니다.</p>

<h3>왜 매년 챙겨야 할까?</h3>
<p>스케일링은 칫솔로 절대 안 떨어지는 ${A('치석')}을 제거하는 유일한 방법입니다. 치석을 방치하면 잇몸병으로 이어지고, 결국 잇몸뼈가 녹아 ${A('발치')}·${A('임플란트')}로 커질 수 있습니다.</p>
<p>비용으로 환산해 보면 더 분명합니다. <strong>연 1회 보험 스케일링은 1만~2만원</strong>입니다. 반면 잇몸병을 방치해 치아를 잃으면 임플란트 1개에 <strong>80만~160만원</strong>이 듭니다. <strong>수십 배 차이</strong>죠. 올해가 가기 전에 꼭 챙기세요.</p>`,
  faqs: [
