...
(function() {
  ...
  var UI_TEXT = {
    ko: {
      ...
      bkSubmit: '예약 신청하기', bkSubmitting: '신청 접수중...',
      bkErrName: '성함을 입력해주세요.', bkErrPhone: '올바른 휴대폰 번호를 입력해주세요.',
      bkErrGeneric: '예약 처리 중 오류가 발생했습니다.', bkErrNetwork: '네트워크 오류입니다. 잠시 후 다시 시도해주세요.',
      bkDone: '예약 신청이 완료되었습니다!',
      bkDoneMsg: function(name, date, time) { return '<strong>' + name + '</strong>님, ' + date + ' ' + time + ' 희망으로<br>예약 신청이 접수되었어요 😊'; },
      bkDoneNote: '금방 확인 연락을 드리겠습니다! 📞',
      bkAiNote: function(name, date, time) { return name + '님, ' + date + ' ' + time + ' 희망으로 예약 신청이 접수되었습니다. 금방 확인 연락드릴게요! 궁금한 점이 더 있으시면 편하게 물어보세요!'; },
      bkMemo: '챗봇 예약',
      days: ['일','월','화','수','목','금','토'],
    ...
    ja: {
      ...
      bkSubmit: '予約を申し込む', bkSubmitting: '受付処理中...',
      ...
      bkDone: 'ご予約の申し込みを受け付けました！',
      bkDoneMsg: function(name, date, time) { return '<strong>' + name + '</strong>様、' + date + ' ' + time + 'ご希望で<br>予約申請を受け付けました 😊'; },
      bkDoneNote: 'まもなく確認のご連絡をいたします！📞',
      bkAiNote: function(name, date, time) { return name + '様、' + date + ' ' + time + 'ご希望で予約申請を受け付けました。まもなく確認のご連絡をいたします。他にご質問があればお気軽にどうぞ！'; },
    ...
    en: {
      ...
      bkSubmit: 'Request Booking', bkSubmitting: 'Submitting...',
      ...
      bkDone: 'Booking request received!',
      bkDoneMsg: function(name, date, time) { return '<strong>' + name + '</strong>, your request for ' + date + ' at ' + time + '<br>has been received 😊'; },
      bkDoneNote: 'We will contact you shortly to confirm! 📞',
      bkAiNote: function(name, date, time) { return 'Booking request received for ' + name + ' (' + date + ' at ' + time + '). We will contact you shortly to confirm. Feel free to ask if you have more questions!'; },
    ...
    zh: {
      ...
      bkSubmit: '提交预约申请', bkSubmitting: '提交中...',
      bkErrName: '请输入姓名。', bkErrPhone: '请输入正确的手机号码。',
      bkErrGeneric: '预约处理时发生错误。', bkErrNetwork: '网络错误，请稍后再试。',
      bkDone: '预约申请已提交！',
      bkDoneMsg: function(name, date, time) { return '<strong>' + name + '</strong>，您希望' + date + ' ' + time + '的<br>预约申请已收到 😊'; },
      bkDoneNote: '我们会尽快与您联系确认！📞',
      bkAiNote: function(name, date, time) { return name + '，您希望' + date + ' ' + time + '的预约申请已收到。我们会尽快与您联系确认。如有其他问题请随时提问！'; },
      bkMemo: '聊天机器人预约 (中文)',
      days: ['日','一','二','三','四','五','六'],
