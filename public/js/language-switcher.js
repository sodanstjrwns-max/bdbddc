...
(function() {
    ...
    const log = IS_PRODUCTION ? () => {} : console.log.bind(console, '[LangSwitcher]');
    ...
    async function initLanguageSwitcher() {
        ...
        // 모바일 네비게이션에 언어 선택 UI 추가
        const mainNav = document.querySelector('.main-nav');
        if (mainNav && !document.querySelector('.mobile-lang-switcher')) {
            const mobileLangSwitcher = createMobileLangSwitcher(currentLang);
            mainNav.appendChild(mobileLangSwitcher);
        }
     ...
     * 모바일 메뉴용 언어 선택 UI 생성
     */
    function createMobileLangSwitcher(currentLang) {
        const container = document.createElement('div');
        container.className = 'mobile-lang-switcher';
        container.innerHTML = `
            <div class="lang-title">🌐 Language</div>
            <div class="mobile-lang-grid">
                ${Object.entries(SUPPORTED_LANGUAGES).map(([code, info]) => `
                    <button class="mobile-lang-btn ${code === currentLang ? 'active' : ''}" 
    ...
    // 초기화 - DOM이 완전히 준비될 때까지 대기
    let initAttempts = 0;
    const MAX_ATTEMPTS = 50; // 5초 동안 시도
    
    function safeInit() {
        initAttempts++;
        log('Init attempt', initAttempts, 'readyState:', document.readyState);
        ...
        if (headerActions) {
            ...
            initLanguageSwitcher().then(() => {
                log('Language switcher initialized successfully');
            }).catch(function(e) { 
                warn('Language Switcher Error:', e); 
            });
        } else if (initAttempts < MAX_ATTEMPTS) {
            // header-actions가 아직 없으면 재시도
            setTimeout(safeInit, 100);
        } else {
            warn('header-actions not found after', MAX_ATTEMPTS, 'attempts');
        }
    }
    ...
    if (document.readyState === 'loading') {
        ...
        log('DOM already loaded, starting init');
