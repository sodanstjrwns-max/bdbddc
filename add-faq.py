...
NEW_FAQS = {
    ...
    "pediatric": [
        ("아이가 치과를 너무 무서워하는데 어떻게 하나요?", "소아치과 전문의 3인이 아이의 공포와 불안을 전문적으로 다룹니다. 첫 방문은 치료 없이 친해지기부터 시작하고, 필요 시 웃음가스나 수면치료로 편안하게 진행합니다. 겁쟁이 아이도 걱정 마세요."),
...
def update_faq_schema(html, new_faqs):
    ...
    pattern = r'("@type":\s*"FAQPage",\s*"mainEntity":\s*\[)(.*?)(\])'
    ...
    if existing.rstrip().endswith('}'):
        new_schema = existing + ", " + ", ".join(new_entries)
    else:
        new_schema = existing + ", ".join(new_entries)
...
def process_file(filepath, faqs):
    ...
    next_id = get_next_faq_id(html)
    
    # 1. HTML FAQ 아이템 추가 (faq-list 닫기 직전에 삽입)
    new_items = []
    for i, (q, a) in enumerate(faqs):
        new_items.append(make_faq_html(q, a, next_id + i))
    
    new_html = "\n".join(new_items)
    
    # faq-list 닫는 태그 직전에 삽입
    closing = "        </div>\n"  # faq-list 닫기
    faq_list_end = html.rfind("</div>", 0, html.find("</section>", html.find("faq-list")))
    
    # 더 정확한 방법: faq-list 내 마지막 faq-item 뒤에 삽입
    last_faq_end = html.rfind("</div>\n          </div>", 0, html.find("</div>\n        </div>", html.find("faq-list")))
    if last_faq_end == -1:
        # fallback: faq-list 닫기 직전
        insert_pos = html.find("</div>\n        </div>", html.find("faq-list"))
        if insert_pos != -1:
            html = html[:insert_pos] + "\n" + new_html + "\n" + html[insert_pos:]
    else:
        insert_pos = last_faq_end + len("</div>\n          </div>")
...
total_added = 0
results = []
for page, faqs in NEW_FAQS.items():
