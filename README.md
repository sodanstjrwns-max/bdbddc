...
- **상태**: 로컬 구현·빌드·브라우저 회귀 검증 완료. 이번 작업으로 운영 배포, 원격 DB 변경 또는 GitHub push를 실행하지 않음.
...
- **추후 확인**: 의료진의 증상–진료 매핑/새 안내 문구 검토, 운영 환경의 동의 문구·분석 설정 점검 후 배포 권장. 기존 예약 저장 실패 시 성공 처리 문제와 `_redirects` 동적 규칙 100개 한도 경고는 이번 범위 밖으로 미변경.
