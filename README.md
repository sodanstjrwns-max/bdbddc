...
**샘플**
```
치주염이란? — 왜 생기나, 무엇을 조심하나 | 서울비디치과
협측이란? — 어느 부위를 가리키는 말인가 | 서울비디치과
구강암이란? — 무엇을 뜻하고 어떻게 진단하나 | 서울비디치과   ← 중대질환 게이트
스케일링이란? — 건강보험 되나요, 비용은 어떻게 되나 | 서울비디치과
지르코니아란? — 어디에 쓰이고 무엇이 다른가 | 서울비디치과
```

**다음 단계(보류).** 2단계로 고가치 30~40종 수동 오버라이드 예정이나 **GSC CSV 2종(top-500 쿼리/페이지, 2026-04-26~07-25) 재업로드가 선행**돼야 함. 효과 판정은 3~4주 후 v5.41 재제목과 함께.
