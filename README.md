...
- **조사 범위**: 정적 HTML·JS·TS/TSX 650파일(관리자·개발 스크립트·보고서 등 제외, 회원 화면 포함), 공개 콘텐츠 JSON 148파일. 정적 HTML 별도 확장 검색은 596파일에서 ‘준비/예정/임시/예시’ 및 다국어 동등 표현을 문맥 검토. 운영 한·일 칼럼 및 블로그 사이트맵의 343개 URL 본문 GET 검사 전부 성공, 대상 미완성 표현 후보 0. 공개 칼럼 목록 114건·사례 106건의 공개 텍스트 필드도 확인. 로그인 제한 사례 사진·원본 저장소/회원 데이터는 변경하지 않음.
- **사진 안내**: 의료진 14명 × 한·영·일 42페이지의 숨김 사진 대체 문구 126개를 ‘사진을 표시할 수 없습니다’ 및 각 언어 안내로 정비. 실제 연결된 이미지 42개 파일 존재 확인. 평소 숨김 안내를 실제 노출 126건으로 과장하지 않음. 이미지 오류 때만 사진을 숨기고 해당 안내를 표시하도록 동작 수정. 사진·의료진 소개 본문 자체는 보존.
- **게임 목록**: 실행 링크가 없는 COMING SOON 카드 2개 제거. 근거 없는 ‘매달 새로운 게임 추가’ 약속 대신 현재 게임 선택 안내. 작동 중인 게임 링크 유지; 미출시 게임을 출시한 것처럼 표시하지 않음.
- **회원 화면**: `/auth/mypage`에서 준비 중 알림만 띄우던 예약 확인/정보 수정/회원 탈퇴 동작을 명시적인 전화 문의 경로로 교체. 저장되지 않는 편집 폼·미구현 삭제 확인창 제거. 실제 연동 없는 예약/방문/포인트 0 표시도 제거. 본인 확인 및 별도 처리 안내를 보이고 즉시 변경/삭제되는 것처럼 안내하지 않음. 회원정보 조회, 비밀번호 변경, 마케팅 동의 변경, 로그아웃은 유지. 온라인 프로필 수정·탈퇴·예약 내역 조회 기능은 이번에 구현한 것이 아님.
- **오류 문구**: 사례 갤러리의 이미지 로드 실패와 사진 미등록 상태를 구분. `/api/chat`에 API 키가 없을 때 ‘준비 중’ 대신 현재 이용 불가 + 대표전화 안내, HTTP 503. 서비스 설정/키를 새로 넣거나 AI 호출을 실행하지 않음. 갤러리 JS 참조 캐시 버전 갱신.
- **정상 표현 유지**: 임시치아·치료 준비·내원 예정·가격 예시·일정 샘플 등 설명에 필요한 문구는 유지. 최종 재검색의 잔여 1후보는 혼합치열 설명의 ‘송곳니·작은어금니 교체를 준비 중’으로, 개발 상태 안내가 아니므로 보존. 개발 기획서의 예시나 관리자 전용 오류 상태를 환자 화면 미완성 안내와 혼동하지 않음.
- **검증**: 전체 빌드 587페이지·오류0·경고0, 언어 묶음 140개. `node scripts/public-copy-test.cjs`로 의료진 42페이지/126사진 오류 처리, 회원 로그인·비로그인, 비밀번호/수신동의의 모의 API 요청, 미출시 게임 카드 제거 확인(런타임 오류0, 실제 외부 쓰기0). 기본 Chromium 경로는 sandbox 설치본이며 다른 환경은 `CHROMIUM_PATH` 지정 및 Playwright 필요.
- **재발 방지**: SEO 빌드 검사에 사진/게임 미완성 안내, 회원 ‘준비 중’ 동작, 폐기된 Amplitude 로더 재유입 검사를 추가. 오류 3종을 dist에 주입해 빌드 차단 확인 후 원복·정상 검사 완료. 의료 내용의 일반 ‘예정’ 단어는 차단하지 않음.
- **상태**: 로컬 병합·문구 수정·검증만 수행. GitHub push 및 Cloudflare 운영 배포는 실행하지 않음. 운영의 외부 위젯·게시글은 계속 변경될 수 있으므로 배포 후 재확인 필요. 이번 조사는 모든 언어/상황의 의미를 완전히 자동 판정하거나 미구현 기능을 완성한 작업이 아님.
...
### 수정 내용
- **실측 범위**: 운영 사이트맵 8개, 1,937개 항목/1,902개 고유 URL을 HEAD 검사하고 비정상 응답은 GET 재확인. 최초 정적 산출물 594페이지 점검. HEAD의 404 5건은 GET 200이므로 콘텐츠 삭제 대신 응답 처리 수정.
- **대표 주소**: `/treatments/`의 canonical·OG·Breadcrumb 및 수면치료 내부 링크를 실제 200 주소로 통일. `/treatments/index`, `/treatments/index.html`, `/doctors/index`, `/doctors/index.html`은 각 허브로 직접 301. `/blog`의 런타임 canonical과 정적 메타·정적/동적 사이트맵도 일치시킴. 기존 `/blog/` 접근은 유지.
- **응답 정확성**: catch-all 자산 검증에서 HEAD는 GET으로 존재를 확인한 뒤 본문 없이 반환하고 자산 리디렉트/304를 보존. 해외 안내 5개 경로의 HEAD 404 해소. 없는 의료진 프로필은 빈 200 대신 명시적인 404 안내.
- **제목 구조**: 50개 페이지의 H2 아래 H4로 건너뛴 카드/단계 제목과 같은 단계의 형제 제목을 H3로 보정. 기존 H3에 딸린 정상 H4는 유지. 한·영·일 안내 허브 및 중·일 가이드 허브에 의미 있는 H2 추가. `heading-from-h4` 선택자와 동일 specificity의 CSS 매핑으로 기존 모양 유지. 변경한 CSS는 빌드에서 내용 해시로 캐시 버전 갱신.
- **사이트맵**: 보험 가이드로 이미 301 통합된 백과사전 주소 3개를 사이트맵에서만 제외(콘텐츠·리디렉트는 유지). 이미지 사이트맵의 `/doctors`를 `/doctors/`로 수정하고 이미지 정보는 보존. 블로그 대표 URL 정리. 동적 목록/인덱스 및 날짜가 없는 문서에 매 요청 오늘 날짜를 넣지 않음; 확인 가능한 원본 변경일만 사용.
- **클린 빌드**: Hono Pages 플러그인의 `emptyOutDir: true` 설정으로 삭제된 소스의 과거 HTML 7개가 다시 배포되는 것을 방지. 현재 검사 대상은 587페이지, 상호 언어 묶음 140개. 페이지 수 감소는 원본 콘텐츠 삭제가 아니라 오래된 산출물 제거.
- **메타 검사**: title·description·canonical·OG URL의 개수/내용/일치 여부, 제목/설명 중복, 본문 제목 계층, 정적 사이트맵 중복·알려진 리디렉트·비색인/비대표 주소, 원본 없는 산출물 검사 추가. 기존 정상 메타를 불필요한 키워드로 덮어쓰지 않음.

### 검증·사용법
- `npm run build:fast`: 현재 587페이지, 차단 오류 0·검토 경고 0. 관리자 인라인 스크립트 23개 검사 유지. 숨김 갤러리 제목, 런타임 게임 결과 제목, 본문 앞 스크럽 애니메이션 자막은 본문 오류로 오인하지 않도록 제한적으로 처리.
- `node --max-old-space-size=256 scripts/seo-release-regression.cjs`: dist에 잘못된 canonical, 중복 description, 제목 단계 누락, 사이트맵 리디렉트를 주입했을 때 모두 차단 확인. 원복 후 정상 및 반복 정규화 동일성 확인.
- `npm run seo:http -- http://localhost:3000`: 빌드 보고서의 정적 공개 URL을 실제 GET/HEAD로 검사. 운영 전체 사이트맵 검사는 `npm run seo:http -- https://bdbddc.com --sitemaps`. 결과는 Git 제외 `.wrangler/seo-http-audit.json`. 두 요청씩, 지연/타임아웃/재시도 적용, 읽기 요청만 수행. 네트워크 실패만으로 URL을 삭제하지 않음.
- `node --max-old-space-size=96 scripts/seo-heading-visual-test.cjs 5728b0f2`: 변경된 제목이 있는 50페이지, 모바일 390px/PC 1440px의 594회 비교에서 검사한 폰트·행간·색상·여백 등 차이 0. sandbox의 Playwright/Chromium 사용; 다른 환경은 Playwright 설치 및 `CHROMIUM_PATH` 지정 필요. 외부 리소스·스크립트·폼 전송 없이 격리 검증.
- 관리 보고서 `/admin/seo-health.json`은 관리자 인증/비공개 유지. 의료진 별칭, 없는 경로 404, 해외 안내 GET/HEAD, 관리자 비로그인 302 및 관리자 API 401 검증. 실제 예약·알림·칼럼 저장/삭제 없음.
- 수정된 HTML의 classic inline JS 137개는 이전 커밋과 바이트 동일. 별도 검사에서 중국어 메인 `cn/index.html`에 **수정 전부터 존재한** 영상 스크립트 문법 오류를 확인했으며 이번 태그 정비에서는 변경하지 않음(후속 기능 수정 대상).

### 반영 상태·한계
- **최종 HTTP 기록**: 587개 URL 전수 실행에서 586개 정상, 임플란트 1개 요청 타임아웃 관찰. 해당 URL을 단독 재검사해 GET/HEAD 모두 200(각 0.04초 미만) 확인. 추가 전수 재실행은 샌드박스 자원 여유가 적어 중단했으며, 운영 배포 후 독립된 환경에서 전체 재검사가 필요. 미리보기는 검증 후 중지. 최종 `seo:check`는 587개·오류0·경고0 통과.
- 이번 기초 정비는 로컬 코드/미리보기 검증 단계이며 **운영 https://bdbddc.com 에는 아직 배포하지 않음**. 기존 본인 Cloudflare 프로젝트 `seoul-bd-dental` 유지, D1/R2/시크릿 변경 없음. 기존 운영 버전은 아래 관리자 칼럼 수정 배포.
- 배포 후 `seo:http --sitemaps` 운영 재검사 및 robots.txt의 이전 캐시 문제 재확인 필요. 실제 검색 색인·순위 상승·의료적 검토·전체 동적 문서의 제목 품질을 보장하는 검사는 아님.
...
- **수정**: `c =>` 복원. 목록 로딩/오류/재시도와 실제 빈 목록/검색 결과 없음 구분, 로드 전 수치 `—` 표시. 401/403만 로그인 이동하고 일시적인 서버 오류는 화면에 안내. 조회수 통계는 목록 표시 후 별도 요청해 실패·지연이 목록을 막지 않도록 처리.
- **API**: 관리자 칼럼 GET은 인증 유지 + private,no-store. R2 미연결·읽기 오류·배열 아닌 데이터는 503으로 표시하고 0건으로 숨기지 않음. 게시/수정/삭제·칼럼 원본 데이터는 변경하지 않음.
- **재발 방지**: 빌드 검사에 관리자 HTML의 classic inline JavaScript 문법 검사를 추가. 오류 발생 시 배포 차단, 보고서 `adminScriptsChecked`에 검사 수 기록.
- **검증**: 운영 칼럼 112건을 읽기 전용으로 받아 격리 브라우저에서 행/카운터/검색 확인. API 503·잘못된 형식·진짜 빈 배열·재시도·조회수 실패 케이스 통과, JS 오류 및 쓰기 요청 0건. 운영 배포 완료: https://1b983aa2.seoul-bd-dental.pages.dev (배포 커밋 `849887a8`). `https://bdbddc.com/admin/columns`의 실제 운영 HTML/API 응답을 격리 브라우저에 중계하여 112행·원장 필터·검색·기존 원고 편집기 열기까지 확인(저장 없이 닫음). 원본 수정/삭제 및 쓰기 요청 없음. 익명 관리자 API 접근은 401 유지. 빌드의 관리자 인라인 스크립트 23개 문법 검사 통과.


## 2026-09-07 — SEO·AEO 1·2·4차 개선 (운영 배포 완료, robots 엣지 캐시 갱신 대기)

### 완료 범위와 사용법
- **1차 기술 정비**: `data/clinic-profile.json`을 메인 검색/공유 메타·병원 운영시간 스키마와 핵심 진료 가이드 시간 안내의 공통 소스로 사용. 메인의 365일 야간진료/14인 전문의 표현을 평일 야간진료/치과의사 협진으로 명확화. 전체 페이지의 모든 서술형 사실·외부 플레이스 정보까지 자동 동기화하는 기능은 아님.
- **릴리스 파이프라인**: `scripts/post-build.cjs`가 `scripts/seo-release.cjs`를 실행. 실제 정적 번역본의 canonical을 기준으로 139개 언어 묶음(303개 페이지)의 상호 hreflang·한국어 x-default를 일치시킴. 없는 번역 페이지를 생성하지 않으며 검색 URL은 유지. 핵심 진료의 중복 MedicalWebPage 문서 2개를 병합. JSON-LD는 raw script 내용으로 검사(HTML 엔티티 디코딩으로 인한 오탐 방지).
- **사이트맵·크롤러**: 정적 페이지 lastmod는 Git 변경일에 근거하고 미커밋 파일은 날짜를 생략. 사이트맵 인덱스의 낡은 고정 lastmod는 제거(동적 R2 문서 날짜는 기존 런타임 유지). 로봇별 그룹에 내부 경로 제외를 명시하고 기존 봇 허용/차단 의도는 유지. robots는 접근 보안 수단이 아님. `_redirects`는 dist에서 정적 93개 → 패턴 26개 순으로 정리하여 기존 동적 규칙 100개 초과 경고를 해소. 현행 `/column/`를 블로그로 보내던 오래된 규칙 제거.
- **2차 핵심 진료**: `/treatments/{implant,sedation,glownate,invisalign}#patient-decision-guide`에 치료 선택·대안·비용 포함 범위·주의사항·의료진과 기존 가이드/수가표를 연결. `css/decision-guide.css`로 기존 디자인과 분리. 새 검색용 페이지나 추가 FAQ 스키마를 대량 생성하지 않음. 주요 단정적 히어로 표현 완화. 기존 영상/몰입 섹션·사례 로그인·증상체커 연결은 유지.
- **4차 운영 점검**: `/admin/stats#seo-operations` (관리자 홈 메뉴에서 진입). 기존 중앙 API의 brandSplit·risers·convEvents·localStats를 이용해 브랜드/비브랜드, 점검 후보, 개별 이벤트, 국내/외국인 접수 기록을 구분해서 표시. 후보는 페이지 노출 100회 이상·CTR 2% 미만 또는 상승 검색어 표본의 노출 30회 이상·평균 순위 4~20위인 경우를 노출순으로 정렬. 내부 점검 규칙이지 검색엔진 기준·순위 예측이 아님. 종류/검색 필터 및 CSV 다운로드 제공(CSV 수식 주입 방어).
- **측정 한계 표시**: 실제 예약 확정·내원·AI 인용 횟수·전체 색인 상태·페이지별 접수 기여도는 현재 API 미제공이므로 미연동/— 표시. 상위 페이지 표본에 없음을 0으로 바꾸지 않음. 기존 통계의 고정 성장 기간·AEO 작동 단정·리드=실제 상담 표현도 정비. GSC/Bing/네이버 원본 도구 링크 제공.
- **기술 검사 보고서**: `/admin/seo-health.json`, 관리자 세션 필요·직원/비로그인 차단·private,no-store·noindex. Worker의 ASSETS로 `dist/admin/seo-health.json`을 읽고 별도 DB를 만들지 않음. 정적 594개 문서의 제목·대표 URL·H1·JSON 문법·언어 연결·새 진료 가이드 링크·비공개 파일 제외 등 검사. 핵심 오류가 있으면 빌드 비정상 종료. 실제 크롤링/색인 상태, 전체 동적 문서, 리치결과 자격, 의학적 검토 또는 CWV 점수는 아님.

### 명령·검증
- `npm run build:fast`: 유튜브 갱신 없이 전체 빌드 + SEO 정규화 + 검사. `npm run build`: 기존 유튜브 갱신 포함.
- `npm run seo:check`: 이미 생성한 dist를 정규화·재검사. 256MB Node heap 제한과 페이지 단위 처리를 사용하며 전체 HTML/DOM을 보관하지 않음.
- 통과: 정적 594개, 차단 오류 0·검토 경고 0(현재 검사 범위). 주요 canonical을 의도적으로 지운 테스트에서 비정상 종료 확인 후 원복·정상 재빌드. 반복 실행 시 HTML·hreflang·사이트맵·리디렉트 결과 동일 확인.
- 통과: 새 통계 모듈 TypeScript 검사, 실측 데이터와 결측/0 구분 단위 테스트, 외부 URL/스크립트 문자열 방어, 관리자/직원/비로그인 접근 테스트. 모바일·PC의 진료 카드/앵커·통계 필터·CSV 다운로드와 기존 증상체커 동의→상담 연결 회귀 검증. 테스트 중 실제 예약·알림 발송 없음.
- **배포**: 원장님 승인으로 기존 Cloudflare Pages `seoul-bd-dental`의 main에 배포. 운영 https://bdbddc.com / 스냅샷 https://7a426b3e.seoul-bd-dental.pages.dev / 배포 커밋 `6578d1a0` (기능 `5a1056f9` + 표준 빌드 영상 캐시 갱신). 이전 배포 `db62946d-902c-42ba-b5b3-2535019f1091`. 별도 DB 마이그레이션·시크릿 변경·GitHub push 없음.
- **운영 검증**: 메인 메타, 핵심 4개 진료 가이드, 한·영·일 임플란트 hreflang 동일, 최신 사이트맵, 기존 증상체커·예약 화면 정상. 관리자 인증으로 실제 `/admin/stats`와 `/admin/seo-health.json`의 새 기능/빌드 보고서 로딩 확인(594개·오류0·배포 커밋 일치·no-store). 비로그인·직원은 접근 차단. 실제 예약은 제출하지 않음.
- **잔여 캐시 이슈**: 배포 스냅샷의 robots.txt는 새 규칙이나 `https://bdbddc.com/robots.txt`는 기존 Cloudflare HIT 캐시가 관찰됨. 해당 파일/사이트맵 6개에 한정한 purge 요청은 현재 토큰에서 HTTP 401/code 10000으로 거부되어 완료되지 않음. Cloudflare에서 robots.txt 단일 URL 수동 purge 또는 Deploy 패널에 Cache Purge 권한 포함 토큰 등록 후 재시도 필요. 캐시 만료로도 갱신될 수 있으나 즉시 반영은 미확인. 관리자 인증에는 영향 없음.
- **다음 단계**: robots.txt 캐시 갱신 확인, 변경 문구의 의료진 검토 및 원본 검색 도구에서 재수집·성과 추적. 3차 의료진 인터뷰·실제 감수 이력은 미실행이며 기존 검토일을 임의로 갱신하지 않음. 병원 정보 전체/다국어 서술·외부 플레이스 동기화, 의료적 근거 전수 검토, 원본 색인/AI 인용 보고서 연동, 실제 예약 확정·내원 연결은 후속 과제.


## 2026-09-07 — 기존 증상체커 노출·안내·상담 연결 개선

- **상태**: 원장님 승인으로 본인 Cloudflare 계정의 기존 `seoul-bd-dental` 프로젝트에 `main` 운영 배포 완료. 운영: https://bdbddc.com / 배포 스냅샷: https://db62946d.seoul-bd-dental.pages.dev / 배포 커밋: `6f8a304c` (기능 변경 `2bd2102b` + 표준 빌드의 YouTube 캐시 갱신). 별도 DB 마이그레이션·시크릿 변경·GitHub push는 실행하지 않음.
- **운영 확인**: 메인·증상체커·예약·분석 JS 최신 내용 HTTP 200, 미인증 `/admin/pricing`·`/admin/stats`는 로그인으로 302. 운영에서 가져온 HTML/CSS/JS를 격리 Chromium에 중계하여 정렬·선택적 전달·일회성 삭제·내용 제거·미동의·모바일 검증 통과. API/외부 추적 요청은 차단했고 실제 예약 제출은 하지 않음.
- **이전 배포**: `b75a4752-3e9b-4782-95a9-0cb1759172e3` / https://b75a4752.seoul-bd-dental.pages.dev (필요 시 Cloudflare 배포 이력에서 롤백).
- **경로**: `/` 진료 안내 상단 `#home-symptom-checker` → `/symptom-checker` → `/reservation?from=symptom-checker`. 기존 콘텐츠 메뉴·사례 접근 권한은 유지.
- **안내 정확성**: 정렬의 `0 || 9` 오류를 `0 ?? 9`로 수정. 화면의 `최적/적합` 배지를 `관련 진료`로 변경하고 진단·치료 확정이 아님을 명시. 일부 단정적 설명 완화, 응급 상황 안내 추가. 증상–진료 매핑은 기존 규칙 기반이며 생성형 AI 호출을 추가하지 않음.
- **사용성**: 이전 버튼의 미노출 함수 참조 오류 수정. 같은 부위에서는 증상 선택을 보존하고 다른 부위 선택 시 초기화. 결과에서 증상 수정 가능. 키보드 선택 상태·단계 초점·reduced-motion 대응.
- **사용 가이드**: 결과에서 선택한 증상을 확인하고 선택 체크박스를 체크한 경우에만 상담 입력란으로 가져옴. 체크하지 않아도 상담 가능. 가져온 내용은 상담 화면에서 수정·삭제하고 기존 개인정보 동의 및 신청 단계를 거쳐 제출.
- **데이터**: 동일 탭의 `sessionStorage.bd_symptom_handoff_v1`에 `{version, expiresAt, summary, treatment}`를 일시 보관. 30분 가져오기 유효기간, 읽을 때 삭제, 잘못된/만료된/과도한 크기 데이터 거절. URL은 출처만 포함. 일반 예약 진입은 전달 내용을 읽어 넣지 않고 정리. 기존 R2 예약 `message` 필드로 사용자가 최종 제출한 내용만 전송하며 새 DB/외부 서비스 없음.
- **개인정보 보호**: 증상 화면·예약 폼에 Clarity 마스킹과 Amplitude 제외 클래스 적용. Amplitude 폴백 초기화에서 해당 경로의 자동 클릭·폼 수집 비활성화. 새 연결 코드에서 증상 원문을 분석 이벤트로 전달하지 않음. 외부 GTM 설정·실제 수집 결과에 대한 전면 감사는 별도 필요.
- **검증**: `npm run build:fast`, `git diff --check`, Chromium Playwright로 6개 부위·35개 개별 증상, 복수 선택 중복 제거, 정렬, 이전·수정, 공유/미공유, 일회성 전달, 만료/손상/크기 초과, 저장 차단 폴백, 삭제 시 수기 내용 보존, 모바일 390px·데스크톱 1440px 검증. 예약 API와 외부 분석 요청은 테스트에서 차단/모의 응답했고 실제 예약 생성·메일 발송은 하지 않음. 테스트 중 페이지 JS 오류 0건.
- **추후 확인**: 의료진의 증상–진료 매핑/새 안내 문구 검토, 운영 환경의 동의 문구·분석 설정 점검 권장. 기존 예약 저장 실패 시 성공 처리 문제와 `_redirects` 동적 규칙 100개 한도 경고는 이번 범위 밖으로 미변경.
...
- **프레임**: 투명교정 애니메이션 67프레임 (`/invisalign/frames/frame_0001~0067.webp?v=577`, 2.5MB) — 정적 서빙 (`_routes.json` exclude), 처음부터 캐시버스터 적용(v5.75 엣지 404 재발 방지)
- **각색**: 라이트 톤, 교정 3단계 스토리텔링 — ① 삐뚤어진 치아의 고민 → ② 티 나지 않는 투명 얼라이너 장착 → ③ 계획된 이동(클린체크) → 피날레 "숨기지 않아도 되는 미소" + 상담 CTA. 단계 라벨(장착—ALIGNER / 완성—SMILE, 후자 72~90% 최종 공개 구간으로 조정)
- **엔진**: v5.75와 동일 (rAF+sticky, contain fit, pass1/pass2, 근접 폴백, 8%~86% 매핑, 560vh/모바일 480vh, reduced-motion 대응) — `iv-*` 프리픽스로 분리
- **검증**: 헤드리스 실측 — sticky 전 구간 pinned, 챕터 1→2→3→피날레 타임라인 정상 (spaceH 4480)
- **크론 참고**: 2026-08-10 컬럼 "미발행" 신고 → 조사 결과 정상 발행(09:03 KST). 1차 시도가 DOI 404 게이트에 차단 후 2차 통과 — 게이트+재시도 설계 정상 작동 확인

## v5.76 — /cases/ 갤러리 서버 렌더링 링크 색인 (2026-08-09)

**문제**: 케이스 상세 페이지 98건은 이미 개별 URL(`/cases/:slug`) + SSR + canonical + `sitemap-cases.xml`을 갖추고 있었으나, 갤러리 카드가 JS `data-href`로만 렌더링되어 **크롤러가 발견할 수 있는 실제 `<a href>` 내부 링크가 0개** — 사이트맵 의존 고아 페이지 상태였음.

- **해결**: `/cases/` 요청 시 Worker가 R2 케이스 목록을 읽어 카테고리별 실링크 색인 섹션(`#case-index`, `<details>` 접힘)을 `</main>` 앞에 서버 주입
- **효과**: 크롤러가 갤러리에서 98개 상세 페이지로 직접 도달 (사이트맵 + 내부 링크 이중 발견 경로)
- **캐시**: `Cache-Control: public, max-age=300` (새 케이스 5분 내 반영)

## v5.75 — 임플란트 페이지 스크롤 스크럽 몰입 섹션 (2026-08-09)

**구조**: `/treatments/implant` 최상단에 글로우네이트(v5.73)와 동일한 스크롤 스크럽 몰입 섹션 추가. 기존 H1 히어로는 아래 100% 원본 유지.

- **프레임**: 3D 임플란트 시술 애니메이션 67프레임 (`/implant/frames/frame_0001~0067.webp?v=575`, 2.0MB) — 정적 서빙 (`_routes.json` exclude)
- **각색**: 라이트 클리니컬 톤(영상 배경 #d6d6d6), 시술 4단계 스토리텔링 — ① 빈자리의 고민 → ② 티타늄 픽스처 식립 → ③ 어버트먼트 정밀 결합 → 피날레 "다시, 내 치아처럼" + 상담 CTA. BEFORE/AFTER 대신 단계 라벨(식립—FIXTURE / 완성—CROWN)
- **엔진**: v5.73과 동일 (rAF+sticky, contain fit, pass1/pass2, 근접 폴백, 8%~86% 매핑, 560vh/모바일 480vh, reduced-motion 대응) — `im-*` 프리픽스로 분리
- **캐시버스터**: 초기 배포 시 엣지에 frame_0001 404가 캐시(max-age=14400)됨 → `?v=575` 쿼리로 우회
- **검증**: 헤드리스 실측 — sticky stagePinned 전 구간(0~95%) true, 챕터 오버레이 타임라인 정상
...
문제: 사이트맵 인덱스에 블로그가 한 건도 없었다. `/blog/*` 는 인블로그(bdbddc.inblog.ai)
리버스 프록시라서 우리 레포에 글 목록이 존재하지 않기 때문이다. 결과적으로 블로그 글
112편이 사이트맵 어디에도 없는 상태였다.

해법: 슬러그를 코드에 박지 않고, 인블로그 원본 `sitemap.xml` 하나를 단일 소스로 삼는다.

- `fetchBlogPosts()` 신설 (src/index.tsx) — 인블로그 사이트맵을 받아 정제한다.
  - `/blog/category/` 258건 제거 (우리가 301로 죽인 경로)
  - `/blog/author/` 6건 제거
  - 인블로그가 뱉는 깨진 퍼센트 인코딩 URL(`%25%25`) 제거
  - 중복 제거 → 실제 글 **112편** 확정
- `GET /sitemap-blog.xml` 신설 — 인덱스 1 + 글 112 = **113건** 동적 생성.
  인블로그 장애 시 빈 배열을 반환해 500 대신 인덱스만 내보낸다.
- 사이트맵 인덱스에 등록. `robots.txt` 에도 추가.

주의점(다음 사람을 위해): `/sitemap.xml` 은 Worker 핸들러가 아니라 **레포 루트의 정적 파일**이
서빙된다. `_routes.json` exclude 에 `/sitemap.xml` 이 들어 있기 때문이다. Worker 쪽 인덱스
핸들러만 고치면 반영되지 않는다. 정적 `sitemap.xml` 을 같이 고쳐야 한다.
반대로 `sitemap-blog.xml` 은 exclude 에 없어야 동적 라우트가 살아 있다. 넣지 말 것.

### 4-2. `/blog/` 인덱스에 크롤 가능한 글 링크 주입

문제: 인블로그 인덱스는 클라이언트 렌더링이라 HTML 안에 글 링크가 **0개**였다.
크롤러가 블로그 글을 발견할 경로가 아예 없었다.

해법: `injectBlogIndexLinks()` 신설. 프록시 응답을 가공하는 지점에서 글 112편 링크를
담은 `<nav id="bd-blog-archive">` 를 주입한다.

- 사이트맵과 **같은 `fetchBlogPosts()`** 를 쓴다. 글이 늘어도 두 곳이 자동으로 같이 늘어난다.
- 화면 중복 노출을 피하려고 시각적으로만 감춘다.
  `position:absolute;width:1px;height:1px;overflow:hidden;clip:rect(0 0 0 0)`
  `display:none` 을 쓰지 않는 이유: 구글은 display:none 링크의 가중치를 낮춘다.
- 인덱스에만 주입한다. 개별 글 페이지에는 경로 가드로 붙지 않는다.
- `bd-blog-archive` 존재 여부로 멱등 처리.

### 4-3. `/encyclopedia/리라이닝` 404 해소

조사 결과 뜻밖의 사실: **같은 개념 항목이 이미 있었다.** `리베이스`(id 172)가 200으로
살아 있고 본문에도 리라이닝이 나오는데, `synonyms` 가 빈 배열이라 검색 유입 URL이
전부 404로 떨어지고 있었다.

레포 전수 검색 결과 `리라이닝` 을 `href` 로 링크한 곳은 **한 군데도 없다.**
GA4가 이 URL을 본 것은 내부 링크가 아니라 외부 유입 또는 검색 결과 잔존으로 판단된다.

해법: 새 항목을 만들지 않고 `리베이스` 의 동의어로 흡수했다. 기존 동의어 처리 로직이
자동으로 대표어로 301을 내주므로 코드 변경이 필요 없다.

추가한 동의어 7개: 리라이닝 / 이장 / 틀니 이장 / 의치 이장 / 틀니 리베이스 / relining / rebase

`public/data/encyclopedia.json` 과 `data/encyclopedia.json` 양쪽 모두 수정했다.
한쪽만 고치면 어긋난다.
...
| 항목 | 이전 | 이후 |
|---|---|---|
| `/sitemap-blog.xml` | 없음(404) | 200 / url 113건 |
| 사이트맵 인덱스 자식 | 6개 | 7개 |
| `robots.txt` Sitemap 줄 | 7 | 8 |
| `/blog/` 안의 글 링크 | **0개** | **112개** |
| 사이트맵 113 URL 응답 | — | **전부 200** (리다이렉트 0 / 오류 0) |
| `/encyclopedia/리라이닝` | 404 | 301 → 리베이스(200) |
| 동의어 7종 | 대부분 404 | 전부 301 → 200 |
| 개별 글에 nav 주입 | — | 0 (인덱스 전용 가드 정상) |
| v5.67 트래킹 5종 | — | 회귀 0 (전 샘플 유지) |

IndexNow 로 `sitemap-blog.xml` 통보 완료.

### 남겨둔 관찰 사항

`public/data/encyclopedia.json` 의 items 820개, `data/encyclopedia.json` 은 838개로
**두 파일의 항목 수가 다르다.** 이번 작업 전부터 있던 상태이고(HEAD 기준 동일) 이번
수정과 무관하다. 다만 어느 쪽이 정본인지 정리가 필요하다.
...
| 버전 URL(`?v=`) | 81 / 81 |
| 중복 키 | 0 |
| 소요 시간 | 약 3분 20초 (편당 약 2.6초) |

### 백필 방문
`POST /api/cron/figure?patch=1` 를 반복 호출. 슬러그를 주지 않으면 「삽화 없는 가장 오래된 발행분 1편」을 자동 선택하고, 다 차면 `{ok:true, done:true}` 를 리턴한다(= 자연 종료 조건).

### 인증 주의
`authed()` 는 `Authorization: Bearer` 가 아니라 **`X-Cron-Secret` 헤더**를 읽는다. Bearer 로 보내면 401.

### Workers AI 안정성 (실직)
75회 생성 중 HTTP 500 이 **7회**(약 9%). 전부 동일 슬러그 재시도로 복구되어 최종 실패 0편. → 밌크 생성은 **반드시 3회 재시도 루프**로 돌린다.

### 검증
R2 `data/columns.json` 기준 삽화 81/81, 버전 URL 81/81, 중복 0. 랜덤 10편 캐시버스터 없이 렌더 확인 → 10/10 `<figure class="col-fig">` 생성, md5 10개 모든 상이. 치아·기구·구조물 들어간 그림 0장.

---

## v5.65 — 삽화 캐시 함정 수정 (2026-08-07)

### 문제
v5.64 로 삽화를 「감정·상황」으로 전면 교재하고 R2 이미지를 새로 생성했으나, 사용자 화면에는 여전히 구 v5.63 임필란트 스토어 그림이 보이는 상태여음.

### 원인 (설계 실수)
`genFigure()` 는 항상 같은 R2 키(`column-figures/<slug>.jpg`)에 덮어쓰면서 `cacheControl: 'public, max-age=31536000, immutable'` 를 부여했다. `immutable` 은 버전이 1년간 재요구하지 않는다는 뜻이므로, URL 이 그대로인 한 이미지를 몇 번 재생성해도 브라우지와 CDN 은 녹은 바이트를 계속 보여준다. 검증 curl 에 `?cb=` 캐시버스터를 붙여서 확인해 이 함정을 몸보지 못했다.

### 수정
`genFigure()` 반환값에만 버전을 붙인다.

```ts
return `/api/images/${key}?v=${Date.now().toString(36)}`
```

- R2 키 자실은 그대로 단일 유지(쓰레기 누적 없음).
- `/api/images/*` 핸들러는 `c.req.path` 를 사용하므로 쿼리스트링이 R2 조회를 깨뜨리지 않는다.
- 생성 시점의 버전이 `bodyFigure` 필드에 저장되어 SSR HTML 에 그대로 나간다.

### ⚠️ 쓸네일은 제외
`genThumb()` 은 원장 지시(「지금 쓸네일은 맘에드니깜 건들지 말고」)에 따라 `?v=` 를 붙이지 않는다. 반환값은 `/api/images/${key}` 그대로 유지.

### 사고 기록 (반복 방지)
1. python heredoc 의 `assert` 가 실패해도 쉘 체이닝이 중단되지 않아, 패치가 적용되지 않은 상태로 배포가 2회 나갔다. → 및드 전에 grep 으로 패치 적용 여부를 무조건 확인한다.
2. `genThumb` 와 `genFigure` 는 바디가 거의 동일해서 `.replace(old,new,1)` 이 앞에 있는 `genThumb` 을 잡았다(= 금지 된 쓸네일을 수정). → 공통 바디 함수는 고유 키 문자열(`column-figures/`)로 `s.index()` 한 다엄 윈도우만 치환한다.

### 검증 (캐시버스터 없이 = 사용자 시점)
삽화 6편 전부 `?v=` 버전 URL 생성 확인, md5 6개 모든 상이(중복 0), 229KB~328KB. `implant-diet-after-surgery` 은 임필란트 스토어가 사라지고 「죽 한 그릇 + 물컵 + 바나나 + 캐릭튰」 로 정상 렌더된다.

---

## v5.64 — 본문 삽화 전면 교체: 구조물 → 감정·상황 (2026-08-07)

### 원장 지적 (정확함)
> 본문에 들어가는 사진은 임필란트 같은 구조물보단 환자들의 걱정 이런 이미지를 다뢬는게 나지 않을까. 귀엽게.
> 구조물이나 진료 같은 삽화들은 다 틀리잖아 그럼 치명적이야.

**맞다.** flux 는 임필란트 스후·근관 단면·잔법·치아 해부를 거의 다 틀리게 그린다.
치사 컴럼에 틀린 구조도가 박혀 있으면 삽화가 아니라 **사고**다. 여기에 「도해」라는 캡션을 달아놓았으니 더 나밼다.

### 바뀐 것
| | v5.63 (폐기) | v5.64 |
|---|---|---|
| 모티프 | 치아·임필란트 구조물 14종 | **감정·상황 12종** (해부 0개) |
| 주인공 | 치아 3~4개 나열 | 둥글고 귀여운 파스텔 클레이 커릭터 |
| 제약 | (없음) | `no teeth, no gums, no jaw, no dental anatomy,`<br>`no syringe, no drill, no dental instruments,`<br>`no blood, no wounds, no realistic human faces` |
| 캡션 | 「이해를 돕기 위한 **도해**」 | 「내용 이해를 돕기 위한 **일러스트**입니다」 |

❗ **하이로 쓸네일(`THUMB_STYLE`)은 건드리지 않았다** — 원장 확정 지식. 삽화(`FIG_STYLE`)만 교체.

### 모티프 12종 (전부 감정·상황)
공포·긴장(심호하는 커릭터+하트) · 통증·부기(볼 짚은 커릭터+아이싱) · 음식(죽 그릇+바나나) ·
기간·경과(벨력+알람시계) · 밤·이갈이(베개+초색달) · 아이·소아(부모-아이 손잡기+곰인형) ·
상담·선택(마주보고 이야기하는 두 커릭터) · 관리·윈생(선반+수건+화불) · 미소·자슰감(거울+반짝이) ·
보장·사후(배지+인증서) · 연구·자료(커릭터+책+돋부기) · 기본(쿠션+마실 물+화불)

### 검증
| 사례 | 선택된 모티프 | 결과 |
|---|---|---|
| 치과 공포증 | 공포·긴장 | 눈 감고 심호하는 커릭터 + 하트 + 담요 ✅ |
| 임필란트 수술 후 음식 | 음식 | 죽 한 그릇 + 밀 + 웃는 커릭터 ✅ |
| 아이 첫 치과 방문 | 아이·소아 | 부모-아이 손잡기 + 곰인형 ✅ |
| 통생교정 vs 채사교정 | 상담·선택 | 마주보고 상담하는 두 커릭터 ✅ |

**한 장도 틀릴 구조물이 없다.** 생성 2.2~3.3초/장 유지.

### 남은 일
- 76편 백필 (하루 2장 자동, 또는 원장 확인 후 일괄)
- 생성 간헐적 500 에러 1회 발생(재시도 성공). Workers AI 일시 오류로 보이지만 발생률 관찰 필요.

## v5.63 — 컬럼 품질 개선 10건 일괄 적용 (2026-08-07)

GSC 실측(컬럼 45p / 2,093클릭 / 65,845노출 / CTR 3.18%)에서 나온 결함 10건을 한 번에 처리.
「안전한 렌더 개선 → 프롬프트 → 데이터 수정 → 이미지」 순서로 진행.

| # | 항목 | 실측 근거 | 수단 | 적용 범위 |
|---|---|---|---|---|
| 1 | 도입부 5유형 순환 | 환자 등장 장면 66/81편(81%), 「지난주 진료실」 47편 | `INTRO_MODES` + `systemPrompt(variant)` | 신규 발행분 |
| 2 | 컬럼→컬럼 본문 문맥 링크 | 인라인 컬럼 링크 0개 | `autolinkColumnBody(siblings, selfSlug, selfTopic)` | 기존 81편 전체(렌더) |
| 3 | 본문 삽화 1장 | 81/81편 본문 이미지 0장 | `genFigure()` + `insertBodyFigure()` + cron 3단계 | 하루 2장씩 백필 |
| 4 | 저성과 3편 제목 교체 | CTR 0.45% / 0.80% / 1.09% | R2 데이터 수정 | 3편 |
| 5 | 소제목 질문형 확대 | 73개 중 13개(17%) | 프롬프트 규칙 | 신규 발행분 |
| 6 | 긴 문장·문단 제한 | 100자↑ 문장 301개, 300자↑ 문단 62개 | 프롬프트 규칙(≤100자 / ≤300자) | 신규 발행분 |
| 7 | 90일↑ 재검토 표기 | 갱신 없음 79편, 90일↑ 6편 | `reviewedAt` + `lastReviewed` JSON-LD + 배지 | 6편 |
| 8 | 상투구 편당 1회 | 「케이스마다 편차가 큽니다」 7편/30회 | 프롬프트 + 게이트 ⑦-b `CLICHES` | 신규 발행분(강제) |
| 9 | 본문 지역·연락처 신호 | 전화 없음 75편, 「천안」 없음 75편 | 프롬프트 규칙 | 신규 발행분 |
| 10 | `/column/references` | 고유 DOI 104개 | 신규 라우트 | 라이브 104편 |

### ② 내부링크 — 네 번 만들어서야 맞췄다 (기록용)
1. **v1 전체 구절 매칭 → 0건.** `focusKeyword` 가 「임플란트 수술 후 붓기」 같은 4어절 구절이라 본문에 그대로 등장하지 않음.
2. **v2 마지막 어절 매칭 → 오배선.** 교정 통증 글의 「통증」이 임플란트 통증 글로 연결됨(도메인 「통증응급」이 겹쳐서).
3. **v3 `overlap >= 2` → 다시 0건.** 과도하게 좁혔음.
4. **v4 주제어 접두 규칙 → 성공.** 형제 키워드의 **첫 어절**이 현재 글의 제목/키워드/카테고리에 등장할 때만 단일 어절 링크 허용.
   - 검증: `implant-diet-after-surgery` 5개 전부 정확(식립·마취·출혈·통증·붓기), `orthodontic-pain…` 0개(오배선 제거).

### ③ 삽화 — h3 가 0개라서 안 들어갔다 (핵심 교훈)
`promoteHeadings()` 는 h2 가 없는 글의 h3 를 h2 로 승격시킨다. 그래서 기존 컬럼은 **h2 7개 / h3 0개**.
`insertBodyFigure()` 가 `/<h3[\s>]/gi` 만 스캔해서 매칭 0건 → 조용히 원본 반환.
→ `/<h[23][\s>]/gi` 로 수정. 세 번째 소제목 앞 삽입, 목차(`buildToc`) 계산 **전에** 호출해 섹션 번호를 흔들지 않음.

- 생성 시간 **2.1~2.4초/장** (125초 상한과 무관 — 별도 요청으로 분리)
- cron 3단계: 오늘 발행분 1장 + 삽화 없는 최고령 1장 = **하루 2장 백필** (81편 약 40일)
- 프롬프트는 히어로 썸네일과 같은 3D 클레이 결, 다만 `tight crop` 으로 상단 여백 제거

### ⑦ dateModified 는 조작하지 않았다
내용을 고치지 않고 `dateModified` 만 올리는 건 구글이 걸러내는 날짜 조작이다.
그래서 새 필드 `reviewedAt` / `reviewNote` 를 만들고 `lastReviewed` JSON-LD + 「YYYY년 M월 D일 재검토」 배지로 정직하게 표기.
(감사 문서에는 5편이라고 적혀 있었지만 실측은 **6편**이었다.)

### 알려진 미해결
- **발행 1회 110초 / 엣지 상한 125초 → 여유 15초.** 진짜 해결책은 본문 생성을 2개 요청으로 쪼개는 것.
- `npm run gate:parity` 18 ≠ 10 불일치는 **v5.63 이전부터 존재**(파이썬 룰이 TS를 못 따라옴). ⑦-b 는 기존 74편에서 탈락 0건.

## v5.62 — 🔴 자동발행 이틀 유실 근본 원인 수정 (2026-08-07)

### 사고
| 날짜 | wallTime | CF status | 실제 결과 |
|---|---|---|---|
| 8/04 | 125,067ms | success | 유실 |
| 8/05 | 125,045ms | success | 유실 (수동 발행으로 메움) |
| 8/06 | 125,074ms | success | **유실** |
| 8/07 | 125,076ms | success | **유실** |

컬럼 80편 정체. `status:success` 는 거짓 보고다 — 워커는 fetch 응답을 받았으므로
예외 없이 끝나고, 실제로는 엣지 응답 상한(125초)에서 잘려 발행 커밋 직전에 죽었다.

### v5.55 진단이 반쪽이었다
당시 「썸네일 61초 + 본문 64초 = 125초」로 보고 2단계 분리했다. 그런데 **본문 생성만으로도
계속 늘어났다** (실측 64s → 79s → 109s, 프롬프트가 v5.59/61 로 커지며 증가).
진짜 범인은 **게이트 탈락 시 같은 요청 안에서 LLM 을 다시 부르는 재시도 루프**였다.
1회 95초 + 재호출 = 무조건 125초 초과.

### 수정
1. `runAutoPublish` 에 시간 예산(`budgetMs=95_000`, `llmEstMs=62_000`) — 예산 초과 시 재호출 안 함
2. `?maxattempts=1` — 요청 1개당 LLM 1회로 제한
3. 크론 워커가 **재시도를 새 요청으로** 돌린다 (최대 3라운드, `scheduled()` 수명 15분)
4. 게이트 지적을 D1 `last_error` 에서 복원 → 요청을 쪼개도 피드백이 이어진다

### 검증 (실물)
- 실제 발행 1회 성공: `microdontia-peg-lateral-waesochi-treatment-options` (110.2초, 게이트 pass, 썸네일 2.5초)
- **컬럼 80편 → 81편**
- 좌초 `processing` 행 3건 회수

## v5.61/62 — 컬럼 품질 개선 7건 (2026-08-06~07)

| 항목 | 조치 | 실측 검증 |
|---|---|---|
| ② DOI 0편 | 7편 확인 (미착수 — 논문 검증 필요) | — |
| ③ 금액 잔존 | 틀니·라미네이트 2편에서 금액 제거 → 「의학적 변수」 표로 대체 | 전 81편 금액 **0개** |
| ④ 결론 요약 박스 | `renderColSummary()` — v5.62 에서 장면 묘사 배제 + 판단 문장만 선별 | 진료실 장면 → 판단 문장으로 교체 확인 |
| ⑤ 의학적 검토 표기 | 이미 존재 (`col-meta-badge rev` + `reviewedBy`) | 라이브 확인 |
| ⑥ 컬럼→진료 동선 | `DOMAIN_TREATMENT` 도메인 우선 배치 + 상위 5개 | 링크 **18개 → 2개** |
| ⑦ 인라인 style | `stripEditorStyles()` 렌더 시점 정리 (R2 원본 불변) | `color:rgb(0,0,0)` **0개** |
| ⑨ 백과→컬럼 역링크 | `encColumnsHtml` (도메인+태그 스코어링, try/catch 보호) | 라미네이트 페이지에 컬럼 **4편** |

R2 백업: `data/columns.backup-20260807.json`


## v5.60 — 관련 컬럼 오배선 수정 (2026-08-06)

### 실측된 버그
`relatedColumns()` 의 유사도 임계값이 `0.3` 이었으나, 80편 규모에서 2-gram 자카드
유사도의 **중앙값은 0.000, 상위권도 0.2 대**였다. 즉 임계값을 통과하는 글이 사실상
없어서 관련 컬럼 6칸이 전부 「오래된 순 채우기」 폴백으로 채워지고 있었다.
→ 라미네이트(심미) 글이 교정·소아·턱관절 글로 링크되던 원인.

### 수정
- `COL_DOMAINS` 진료 도메인 15종 태그 도입 (심미/임플란트/교정/사랑니/신경치료/보철/
  잇몸/충치/소아/턱관절/구강관리/통증응급/외상/습관). 슬러그(영문)+제목(한글) 동시 매칭.
- 도메인 교집합 1개당 `+1.2` 가산, 교집합이 있으면 유사도 무관 통과.
- 임계값 `0.3 → 0.18`.
- 전 컬럼이 `category='진료 이야기'` 단일값이라 신호가 0인 카테고리 가산(+0.4) 제거.

### 검증 (배포 전 오프라인 시뮬레이션 → 배포 후 실물 일치 확인)
| 대상 | 개선 전 | 개선 후 |
|---|---|---|
| 라미네이트 글 | 교정·정중선·턱관절·소아 | **미백·변색·시림 5편** |
| 임플란트 식사 | 혼재 | **임플란트 6편 전부** |
| 사랑니 회복 | 혼재 | **사랑니 4편 + 통증 2편** |
| 도메인 커버리지 | — | 80/80편 (100%) |
| 인링크 고아 | 69편(v5.49 이전) | **0편** (중앙값 6, 최대 15) |

---

## v5.59 — 비용·금액 주제 3중 차단 (2026-08-06, 원장 지시)

### 배경
논문 DOI 를 인용하는 의학 정보 글에 진료비를 적으면 **논문이 가격의 근거처럼 읽혀**
글 전체의 신뢰가 무너진다. 또한 금액은 반년이면 틀린 정보가 되어 SEO 자산이 부채가 된다.

### 3중 방어
1. **큐 필터** (`src/column-auto.ts`) — 후보 선정 전 비용성 검색어 17개 키워드를
   `status='skipped_cost'` 로 격리. 기록은 보존한다.
2. **프롬프트** (`systemPrompt()`) — 금액 표기 금지, 대신 「비용을 좌우하는 의학적 변수」
   (재료·치아뿌리 상태·뼈이식 필요 여부·회차·유지관리)를 쓰도록 지시. 상담 유도 문구 제공.
3. **게이트** (`src/column-gate.ts`) — `MONEY_PATTERNS` 5종으로 발행 직전 차단.
   대표번호(041-415-2892)·「10년 보증」 같은 비금액 숫자는 걸리지 않도록 화폐 단위 필수.
   title/metaTitle/metaDescription 에 「비용·가격·얼마·만원·금액·수가」 포함 시 차단.

### 실측 결과
- 비용성 pending **15건 격리** (틀니 비용 220.6 / 치과 실비보험 130.9 / 신경치료 가격 110.1 …)
- 좌초 행 1건 회수 → `pending 156 · published 6 · skipped_cost 15`
...
## v5.58 — 자동발행 진짜 원인 2건 (2026-08-05)

이틀 연속(8/4·8/5) 미발행. v5.55 수정은 **원인 진단이 틀렸다.**

### 원인 ① 크론 워커 실행 하드 상한 125초
```
8/5 00:00  success  wallTime 125.044초
8/4 00:00  success  wallTime 125.067초
8/3 00:00  success  wallTime 125.086초
```
세 번 모두 **정확히 125.0초**. 엣지 상한(100초)이 아니라
`scheduled()` 실행 자체의 하드 상한이었다. v5.55 의 "3회 재시도" 는
1회차에서 워커가 통째로 죽으므로 **아예 실행되지 못했다.**

**수정**: 한 요청에 다 못 넣으니 2단계로 쪼갠다.
- 1단계 `?nothumb=1` 본문만 발행 — **실측 74.9초**
- 2단계 `/api/cron/thumb?patch=1` 썸네일 — **실측 2.2초**

### 원인 ② R2 덮어쓰기 경합 (조용한 유실)
`runAutoPublish` 는 `columns.json` 을 **작업 시작 시점에 읽고**,
약 75초 뒤 그 스냅샷에 append 해서 **통째로 put** 했다.
그 사이 다른 발행이 끼면 결과가 조용히 사라진다.
실측: `verdict:"pass"`, `ok:true` 인데 컬럼 수가 79편에서 늘지 않았고,
큐 행만 `draft`/`processing` 으로 남았다. 로그상 성공이라 놓쳤다.

**수정**: R2 `put` **직전에 다시 읽어** 최신 배열에 append.
같은 slug 가 이미 있으면 덧붙이지 않고 기존 발행을 유지한다.
- 파일: `src/column-auto.ts` (⑤ 발행 블록)

### 검증
| 항목 | 결과 |
|---|---|
| 좌초 큐 행 회수 | 2건 (`라미네이트 후회`, `왜소치`) → pending |
| 1단계 발행 | 109.8초 · `laminate-veneer-regret-causes-checklist` |
| 컬럼 수 | **79 → 80편** ✅ (경합 수정 확인) |
| 2단계 썸네일 | 2.2초 · `patched:true` |
| 크론 워커 완주 | **0.35초** — 오늘 발행분 감지해 즉시 종료 |
| 큐 잔량 | pending 170건 (약 170일분) |
