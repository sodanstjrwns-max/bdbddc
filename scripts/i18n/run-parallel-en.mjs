#!/usr/bin/env node
/**
 * EN 병렬 배치 러너: 페이지 N개 동시 번역 (기본 5)
 * 사용: node scripts/i18n/run-parallel-en.mjs <batch.json> [concurrency]
...
// en-sitemap 누적 등록
const smPath = path.join(__dirname, 'en-sitemap.json')
let sm = new Set()
try { sm = new Set(JSON.parse(fs.readFileSync(smPath, 'utf8'))) } catch {}
for (const b of batch) {
  const ko = b.path.replace(/^\/en/, '') || '/'
...
async function worker(id) {
  while (idx < queue.length) {
    ...
    await new Promise((resolve) => {
      const p = spawn('node', [path.join(__dirname, 'translate-page-en.mjs'), b.src, b.out, b.path], {
